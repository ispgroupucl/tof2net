...
class StochasticDepthDropout(nn.Module):
    """Dropout for RESIDUAL blocks only (https://arxiv.org/pdf/1603.09382.pdf)
        Implementation taken from official Efficientnet github

    Arguments:
        nx (Tensor): Input of this structure.
        p (float): Probability of dropped sample.
    ...
    def forward(self, nx):
        ...
        random_tensor += torch.rand([batch_size, 1, 1, 1], dtype=nx.dtype, device=nx.device)
...
def get_all_blocks():
    ...
    return {
        ...
        'residual2': lambda *args, **kwargs: ResidualBlock(*args, **{**kwargs, "v2":True}),
        'double': DoubleBlock,
        'simple': SimpleBlock,
    }
...
def create_cba(model_cfg):
    ...
    class BasicBlock(nn.Module):
        ...
        """
        def __init__(self, in_width, width, ksize, bn=True, act=True,
                        reverse=False, bias=True, padding_type='zeros',
                        stride=1, depthwise=False, dilation=1):
            super(BasicBlock, self).__init__()
            if depthwise:
                assert(in_width==width), "Input & output width should be equal for depthwise convolutions"
            ...
            padding = ksize // 2
            conv = _Conv(
                in_width, width, ksize, stride=stride,
                dilation=dilation,
                padding=padding if ksize % 2 != 0 else 0,
                bias=bias, padding_mode=padding_type,
                groups=in_width if depthwise else 1
            )
            if ksize % 2 == 0 and padding > 0:
                conv = nn.Sequential(
                    nn.ZeroPad2d((padding, padding-1, padding, padding-1)), # TODO: check if 0's should not be padding-1
                    conv
                )
...
class DoubleBlock(nn.Module):
    def __init__(self, model_cfg, in_width, width, ksize,stride=1, bn=True, act=True, **kwargs):
...
class ResidualBlock(nn.Module):
    ...
    Arguments:
        in_width (int): the number of input channels
        width (int): the number of output channels
        ksize (int): the size of the convolutional channel
        stride (int): the size of the stride, if >1 it will down-sample the image
        bn (bool): wether to use batch normalization or not
        act (bool): wether to use an activation function or not
        v2 (bool): wether to use v2 residual blocks
        se_ratio (float): squeeze-and-excite ratio
        sddrop (float): probability of stochastic depth

    Returns:
        A residual layer
    """
    def __init__(self, model_cfg, in_width, width, ksize, stride=1,
                 bn=True, act=True, v2=False, se_ratio=0., sddrop=0,
                 **kwargs):
        super().__init__()
        CBA = create_cba(model_cfg)
        ...
        self.block1 = CBA(in_width, width, ksize, bn=bn, act=act, reverse=v2, stride=stride)
        self.block2 = CBA(   width, width, ksize, bn=bn, act=act if v2 else False, reverse=v2)

        self.sdd = StochasticDepthDropout(p=sddrop) if sddrop>0 else lambda xx: xx

        self.id_block  = CBA(in_width, width, 1, bn=bn, act=False, stride=stride) if (in_width != width) or stride>1 else \
                            lambda xx: xx
...
class MobileBlock(nn.Module):
    def __init__(self, model_cfg, in_width, width, ksize, bn=True, act=True, stride=1,
                 first=False, residual=True, widen=2, se_ratio=0, sddrop=0,
                 **kwargs):
        ...
        self.s_conv = nn.Sequential(
            CBA(in_width,  mid_width,     1, bn=False if first else bn,
                act=False, reverse=True),
            CBA(mid_width, mid_width, ksize, bn=bn, act=act, reverse=True,
                depthwise=True, stride=stride),
